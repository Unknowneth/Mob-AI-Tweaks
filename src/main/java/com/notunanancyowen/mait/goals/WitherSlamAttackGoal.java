...
public class WitherSlamAttackGoal extends Goal {
    ...
    public WitherSlamAttackGoal(WitherEntity mob) {
        this.mob = mob;
    }
    ...
    @Override public void tick() {
        int i = ((WitherAttacksInterface)mob).getSlamTime();
        if(slamTime != i) slamTime = i;
        ...
        if(slamCooldown > 0) slamCooldown--;
            ...
            if(slamTime < 20) {
                if(slamTime == 1) {
                    mob.getWorld().playSoundFromEntity(mob, SoundEvents.ENTITY_TNT_PRIMED, mob.getSoundCategory(), 3f, mob.getSoundPitch());
                ...
                if(slamTime == 20) {
                    mob.getWorld().playSoundFromEntity(mob, SoundEvents.ENTITY_BREEZE_WHIRL, mob.getSoundCategory(), 3f, mob.getSoundPitch());
                    slamTo = (target.getPos().subtract(target.getVelocity().multiply(4d))).subtract(mob.getPos()).normalize().multiply(2.5d);
                }
