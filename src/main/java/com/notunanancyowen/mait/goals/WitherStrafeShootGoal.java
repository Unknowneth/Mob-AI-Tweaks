...
public class WitherStrafeShootGoal extends Goal {
    ...
    public WitherStrafeShootGoal(WitherEntity mob) {
        this.mob = mob;
    ...
    @Override public void tick() {
        if(mob.getHealth() > mob.getMaxHealth() * 0.5f && mob.getTarget() instanceof LivingEntity target) {
            ...
            Vec2f strafeMovement = new Vec2f((float)target.getPos().getX(), (float)target.getPos().getZ()).add(new Vec2f(-(float)mob.getPos().getX(), -(float)mob.getPos().getZ())).normalize().multiply(0.4f);
            ...
            mob.setVelocity(strafeMovement.x, Math.signum(target.getPos().getY() + 6 - mob.getPos().getY()) * 0.4d, strafeMovement.y);
        }
        else strafingTime = 0;
    }
}
