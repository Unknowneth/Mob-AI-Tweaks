...
public class PillagerEatItemToHealGoal extends Goal {
    ...
    @Override public boolean canStart() {
        for (int i = 0; i < mob.getInventory().size(); i++) if(mob.getInventory().getStack(i).getComponents().contains(DataComponentTypes.FOOD)) return mob.getHealth() < mob.getMaxHealth() && !mob.isUsingItem() && mob.getTarget() == null;
        return false;
    }
    @Override public boolean canStop() {
        if(offCombatTimer < 0) return true;
        for (int i = 0; i < mob.getInventory().size(); i++) if(mob.getInventory().getStack(i).getComponents().contains(DataComponentTypes.FOOD)) return mob.getHealth() >= mob.getMaxHealth() || mob.getTarget() != null || !mob.isUsingItem();
        return true;
    }
    ...
    @Override public void tick() {
        ...
        if (mob.getStackInHand(whichHand).getItem() != null && mob.getStackInHand(whichHand).getComponents().contains(DataComponentTypes.FOOD)) {
            mob.setCurrentHand(whichHand);
            if(mob.getItemUseTimeLeft() == 1) {
                var foodToEat = mob.getStackInHand(whichHand).get(DataComponentTypes.FOOD);
                if(foodToEat != null) mob.heal(foodToEat.nutrition());
                mob.tryEatFood(mob.getWorld(), mob.getActiveItem());
                mob.stopUsingItem();
                offCombatTimer = -1;
            }
