...
import com.notunanancyowen.mait.goals.WitherChargeAttackGoal;
import com.notunanancyowen.mait.goals.WitherSlamAttackGoal;
import com.notunanancyowen.mait.goals.WitherStrafeShootGoal;
...
public abstract class WitherEntityMixin extends HostileEntity implements SkinOverlayOwner, RangedAttackMob, WitherAttacksInterface {
    ...
    private void trackData(DataTracker.Builder builder, CallbackInfo ci) {
        ...
        builder.add(SLAM_TIME, 0);
        builder.add(ENRAGED, false);
        builder.add(BLUE, false);
    }
    @SuppressWarnings("all")
    @Inject(method = "initGoals", at = @At("TAIL"))
    private void addNewAttacks(CallbackInfo ci) {
        if(!MobAITweaks.getModConfigValue("wither_rework")) return;
        goalSelector.add(0, new WitherChargeAttackGoal((WitherEntity)(HostileEntity)this));
        goalSelector.add(0, new WitherSlamAttackGoal((WitherEntity)(HostileEntity)this));
        goalSelector.add(1, new WitherStrafeShootGoal((WitherEntity)(HostileEntity)this));
    ...
    private void flyUpToChasePlayer(CallbackInfo ci) {
        if(isReworked && !isAiDisabled() && getChargeTime() <= 0 && getSlamTime() <= 0 && getInvulnerableTimer() <= 0 && getTarget() instanceof LivingEntity target) {
            double speedMultiplier = distanceTo(target) > 12 ? (target.isFallFlying() ? 3.2d : 1.7d) : 1.1d;
            double yVelocity = Math.abs(getVelocity().y);
            setVelocity(getVelocity().getX() * speedMultiplier, (target.getY() > getEyeY() ? yVelocity : -yVelocity) * (Math.abs(target.getY() - getEyeY()) > 8 ? 2.8d : 1.2d), getVelocity().getZ() * speedMultiplier);
            getLookControl().lookAt(target);
            lookAtEntity(target, 30f, 30f);
        }
        if(getInvulnerableTimer() > 0) setVelocity(Vec3d.ZERO);
    }
    ...
    private void putNewStuffToNBT(NbtCompound nbt, CallbackInfo ci) {
        if(deathTime > 0) nbt.putBoolean("DropsNetherite", MobAITweaks.getModConfigValue("wither_boss_drops_netherite"));
    ...
    private static DefaultAttributeContainer.Builder attribution(DefaultAttributeContainer.Builder original) {
        if(MobAITweaks.getModConfigValue("wither_rework")) return original.add(EntityAttributes.GENERIC_ARMOR, MobAITweaks.getModConfigValue("wither_boss_armor", 12)).add(EntityAttributes.GENERIC_FOLLOW_RANGE, MobAITweaks.getModConfigValue("wither_boss_follow_range", 120)).add(EntityAttributes.GENERIC_MAX_HEALTH, MobAITweaks.getModConfigValue("wither_boss_health", 300));
        return original;
    }
    @Override public boolean damage(DamageSource source, float amount) {
        lastDamageSource = source;
        if(blockBreakingCooldown >= 20) blockBreakingCooldown = 0;
        if(isInvulnerableTo(source) || (getInvulnerableTimer() > 0 && !source.isIn(DamageTypeTags.BYPASSES_INVULNERABILITY)) || source == getDamageSources().mobAttack(this) || source.isOf(DamageTypes.BAD_RESPAWN_POINT) || source.isOf(DamageTypes.IN_FIRE) || source.isOf(DamageTypes.ON_FIRE) || source.isOf(DamageTypes.CAMPFIRE) || source.isOf(DamageTypes.LAVA) || source.isOf(DamageTypes.WITHER) || source.isIn(DamageTypeTags.WITHER_IMMUNE_TO) || source.getAttacker() == this || source.getAttacker() instanceof WitherEntity) return false;
        else if(source.isOf(DamageTypes.WIND_CHARGE)) amount += (float)getAttributes().getBaseValue(EntityAttributes.GENERIC_ARMOR) / 3f;
    ...
    @Override protected void updatePostDeath() {
        if(!MobAITweaks.getModConfigValue("wither_death_animation") || lastDamageSource.isOf(DamageTypes.GENERIC_KILL)) {
        ...
        if(++deathTime % 10 == 0) {
            if(shouldDropXp() && lastDamageSource != null && lastDamageSource.getAttacker() != null) dropXp(lastDamageSource.getAttacker());
        ...
        if(deathTime == totalDeathTime - 1) {
            ...
            if(getDeathSound() instanceof SoundEvent sound) playSound(sound, getSoundVolume() * 1.3f, getSoundPitch());
        }
        else if(!isRemoved() && deathTime >= totalDeathTime) {
            if(lastDamageSource != null && lastDamageSource.getAttacker() != null && getServer() != null) drop(getServer().getWorld(getWorld().getRegistryKey()), lastDamageSource);
            getWorld().emitGameEvent(this, GameEvent.ENTITY_DIE, getPos());
            remove(RemovalReason.KILLED);
        }
    }
    @Override protected void drop(ServerWorld world, DamageSource damageSource) {
        if(!MobAITweaks.getModConfigValue("wither_death_animation") || isEnraged() || deathTime >= DEATH_TICKS * 10) super.drop(world, damageSource);
    }
    @Override protected void setRotation(float yaw, float pitch) {
        super.setRotation(isReworked && (getSlamTime() > 0 || getChargeTime() > 0) ? (float)serverYaw : yaw, pitch);
    }
    @ModifyConstant(method = "tickMovement", constant = @Constant(doubleValue = 0.05))
    private double doNotRotate(double constant) {
        if(getSlamTime() > 0 || getChargeTime() > 0) return constant;
        if(isReworked || getWorld().isClient) return Float.POSITIVE_INFINITY;
        return constant * 2;
    }
    @SuppressWarnings("all")
