...
import com.notunanancyowen.mait.goals.ElderGuardianDashAttackGoal;
...
public abstract class ElderGuardianEntityMixin extends GuardianEntity implements SpecialAttacksInterface {
    ...
    private static DefaultAttributeContainer.Builder attribution(DefaultAttributeContainer.Builder original) {
        ...
        return original.add(EntityAttributes.GENERIC_ARMOR, MobAITweaks.getModConfigValue("elder_guardian_boss_armor", 8)).add(EntityAttributes.GENERIC_MAX_HEALTH, MobAITweaks.getModConfigValue("elder_guardian_boss_health", 150));
    }
    ...
    @Override public boolean damage(DamageSource source, float amount) {
        if(source.isOf(DamageTypes.EXPLOSION) || source.isOf(DamageTypes.FALL) || source.isOf(DamageTypes.MAGIC) || source.getAttacker() == this) return false;
    ...
    @Override protected void updatePostDeath() {
        ...
        if(deathTime == totalDeathTime - 1) {
            if(getDeathSound() instanceof SoundEvent sound) playSound(sound, getSoundVolume(), getSoundPitch());
        }
